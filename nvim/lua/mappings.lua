...
vim.keymap.set("v", "<leader>r", function()
  -- schedule loading so visual mode survives
  vim.schedule(function()
    require("lazy").load({ plugins = { "sniprun" } })
    vim.api.nvim_feedkeys(
      vim.api.nvim_replace_termcodes("<Plug>SnipRun", true, false, true),
      "x",
      false
    )
  end)
end, { silent = true, desc = "SnipRun chunk only" })
vim.api.nvim_set_keymap('v', '<leader>x', '<Plug>SnipClose', { silent = true })
vim.api.nvim_set_keymap('v', '<leader>s', '<Plug>SnipReset', { silent = true })
...
-- })
--
-- -- Auto-load SnipRun and run a selected visual chunk
-- vim.keymap.set("v", "<leader>r", function()
--   -- Load SnipRun if not loaded
--   local ok, sniprun = pcall(require, "sniprun")
--   if not ok then
--     vim.cmd("packadd sniprun")     -- load the plugin
--     sniprun = require("sniprun")
--     sniprun.setup({
--       selected_interpreters = {},                            -- auto-select interpreters
--       display = { "VirtualTextOk", "VirtualTextErr" },       -- show result inline
--     })
--   end
--
--   -- Run only the selected chunk
--   vim.cmd("'<,'>SnipRun")
-- end, { silent = true, desc = "Run selected snippet" })
--
-- -- Close SnipRun outputs
-- vim.keymap.set("v", "<leader>x", function()
--   local ok, sniprun = pcall(require, "sniprun")
--   if ok then sniprun.close() end
-- end, { silent = true, desc = "Close SnipRun outputs" })
--
-- -- Reset SnipRun
-- vim.keymap.set("v", "<leader>s", function()
--   local ok, sniprun = pcall(require, "sniprun")
--   if ok then sniprun.reset() end
-- end, { silent = true, desc = "Reset SnipRun" })
--
